...
local outModelFile = "model_count_01_" .. typeInput;

os.execute("mkdir logs"); -- create log directory from terminal

require './models/model_count_01.lua';
model:training();

local inSize, inDim, outSize = 192, 2, 192;
local batchSize = 32;
local numEpochs = 70;
----------------------------------------------------
local pathTrainOrtho = '../../data/03_biomass/train/ortho_aug_dat';
local pathTrainDem = '../../data/03_biomass/train/dem_aug_dat';
local pathTrainLabels = '../../data/03_biomass/train';
----------------------------------------------------
local labelFileName = 'countTrain_aug.mat';

local numThreads = batchSize/2; -- cpu thread

-- specify log file
trainLogger = optim.Logger('./logs/train_count_resnet_01_osavi_part1.log');
...
local optimState = { -- for sgd
	learningRate = 0.0001, -- 0.001(1-11), 0.0001(11-71), 
	weightDecay = 0.0001
}
...
local numFiles = #paths.dir(pathTrainDem) - 2;
...
function train()
	...
	print(string.format("Total Biomass Count = %d", dataLabels:sum()));
			...
			for fileId = batch_st, batch_ed, 1 do
				donkeys:addjob(
					function()
						...
						local ortho = torch.load(paths.concat(pathTrainOrtho, tostring(fileList[fileId]) .. '.dat'));
						local dem = torch.load(paths.concat(pathTrainDem, tostring(fileList[fileId]) .. '.dat'));
						inputsCpu[placeId] = torch.cat(ortho, dem, 1);
